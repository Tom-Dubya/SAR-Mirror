...
class DOW50001(nn.Module):
    def __init__(self, num_classes : int):
        super(DOW50001, self).__init__()

        self.pool = nn.AvgPool2d(kernel_size=2, stride=2, padding=0)

        self.features = nn.Sequential(
            ConvBlock(1, 16, depth=2, kernel_size=5, stride=1, padding=1, batch_norm=True),
            ConvBlock(16, 32, depth=2, kernel_size=3, stride=1, padding=1, batch_norm=True),

            ResidualBlockDownSample(32, 64, depth=1, kernel_size=3, batch_norm=True),
